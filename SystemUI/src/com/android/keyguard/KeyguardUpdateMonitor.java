...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    //changed to public by wumin
    public static final int FINGERPRINT_STATE_STOPPED = 0;

    /** Fingerprint state: Listening. */
    //changed to public by wumin
    public static final int FINGERPRINT_STATE_RUNNING = 1;
     ...
     */
    //changed to public by wumin
    public static final int FINGERPRINT_STATE_CANCELLING = 2;
    ...
    //changed to public by wumin
    public int mFingerprintRunningState = FINGERPRINT_STATE_STOPPED;

    //add by wumin
    private boolean keyguardIsShowing = false;
    private boolean keyguardOrBouncerIsReady = false;
    private AudioManager mAudioManager;
    private boolean mOccluded;
    private boolean isMusicActive = false;
    private boolean musicLockScreenActive;
    private ActivityManager mActivityManager;
    private String[] musicPackage = {"com.tencent.qqmusic","cn.kuwo.player","com.kugou.android","fm.xiami.main","cmccwm.mobilemusic","com.netease.cloudmusic"
                            ,"com.sds.android.ttpod","com.gwsoft.imusic.controller","com.duomi.android"};
    private final String PERSIST_NAVIGATION_BAR = "persist.sys.navg_bar_visible";
    private static final String FINGERPRINT_UNLOCK_SCREEN_ONSLEEP = "persist.sys.fp_unlock_onsleep";
    public static boolean applock_cancle_verify = false;
    public static boolean applock_verify = false;
    private static final String FINGERPRINT_UNLOCK_SCREEN = "persist.sys.fingerprint_unlock";
    public static boolean FingerprintUnlockEnable;
    ...
    //changed to public by wumin
    public void setFingerprintRunningState(int fingerprintRunningState) {
	...
	if(mActivityManager != null){
            ...
            for(int i=0;i<musicPackage.length;i++){
            	if(mComponentName.getPackageName().equals(musicPackage[i])){
	...
	if(SystemProperties.getBoolean(FINGERPRINT_UNLOCK_SCREEN_ONSLEEP, false)){
	    ...
	    //}else{
	    	keyguardOrBouncerIsReady = (mKeyguardIsVisible || !mDeviceInteractive || mBouncer || mGoingToSleep) && isFingerUp;
	    //}
	}else {

	if((keyguardIsShowing) || mBouncer){
	    if(keyguardIsShowing && mDeviceInteractive){
	    	keyguardOrBouncerIsReady = true;
		if(mOccluded){
	    	    keyguardOrBouncerIsReady = false;
        ...
        return  keyguardOrBouncerIsReady && FingerprintUnlockEnable
    ...
    private void stopListeningForFingerprint() {
        ...
        //this line be modified by wumin
        if (mFingerprintRunningState == FINGERPRINT_STATE_RUNNING && mFingerprintCancelSignal != null) {
            mFingerprintCancelSignal.cancel();
